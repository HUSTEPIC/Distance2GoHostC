 ...
 * to messages recieved from the host.
 *
 ...
 * additional payload messages preceeding that status message. Once the device
 ...
 * Endpoints are continously enumarated starting with 1. At least one logical
 ...
 * information. The moduce \ref Protocol.c further also implements functions
 ...
 * recommendod to call \ref com_get_port_list to get a list of available COM
...
/**
 * \brief This function returns the firmware version of the connected device.
 *
 * This function sends a message to the connected device to query the firmware
 * version and waits for the answer. The reply message is evaluated and
 * firmware version information is written into the provided array.
 *
 * \param[in]  protocol_handle  A handle to an open connection returned.
 * \param[out] version          A pointer to the array, where the firmware
 *                              version will be stored.
 *
 * \return If the provided handle is valid, 0 will be returned and the
 *         firmware version will be written to pVersion. Otherwise a negative
 *         error code is returned and pVersion will not be modified.
 */
int32_t protocol_get_firmware_version(int32_t protocol_handle,
                                      uint16_t (*version)[3]);
