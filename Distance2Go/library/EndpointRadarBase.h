 ...
 * sensor board sends divice informatio, a structure of this type is returned
 ...
 *        radar data frame.
 *
 * A struture of this type must be passed to
 ...
 * \param[in] tx_power         The power value the constant tone is emmitted
...
/**
 * \brief This function queries the current RF trensmission power from the
 *        sensor device.
 *
 * This function sends a message to the specified endpoint to query the
 ...
 * \param[in] endpoint         The endpoint in the connected device to send
 *                             the message to.
 * \param[in] tx_antenna       The number of the TX antenna whose transimmion
 *                             power is requested.
 *
 * \return If a status message is received from the device the function
 *         returns a positive number containing the 16 bit status code in
