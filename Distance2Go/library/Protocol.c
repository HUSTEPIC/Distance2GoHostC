 ...
 * @{
 */
#define CNST_STARTBYTE_DATA           0x5A    /**< A padload message begins
                                                   with this code byte. */
...
#define CNST_MSG_QUERY_FW_VERSION     0x01    /**< This command code sent to
                                                   endpoint 0 tells the device
                                                   to send its firmware
                                                   version. */
#define CNST_MSG_FW_VERSION           0x01    /**< A message containing the
                                                   firmware version sent from
                                                   endpoint 0 starts with this
                                                   message type code. */
...
typedef struct
{
    int32_t     com_port_handle;  /**< A handle to an open COM port. */
                                      ...
                                      uint16_t (*version)[3])
{
     ...
     */
    {
        uint8_t uQueryMessage[1] = { CNST_MSG_QUERY_FW_VERSION };
    ...
    if ((receive_status != CNST_PROTOCOL_RECEIVED_PAYLOAD_MSG) ||
        ...
        (message_info.payload[0] != CNST_MSG_FW_VERSION))
    {
        /* This is not the expected payload, clean up and quit. */
        free(message_info.payload);
        return PROTOCOL_ERROR_DEVICE_NOT_COMPATIBLE;
    ...
    (*version)[0] = (uint16_t)message_info.payload[1] |
                   ((uint16_t)message_info.payload[2] << 8);
    (*version)[1] = (uint16_t)message_info.payload[3] |
                   ((uint16_t)message_info.payload[4] << 8);
    (*version)[2] = (uint16_t)message_info.payload[5] |
                   ((uint16_t)message_info.payload[6] << 8);
                                                 ...
                                                 int32_t status_code)
{
    ...
    if (status_code == PROTOCOL_STATUS_OK)
        return "No Error.";
